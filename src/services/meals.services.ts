...
export const fetchMealData = async (searchTerm: string) => {
  const response = await fetch(
    `${apiurl}/api/v2/search?categories_tags_en=${encodeURIComponent(searchTerm)}&fields=code,product_name,product_name_ar,nutriments&page_size=20`,
    {
      headers: {
  ...
  const formattedProducts = data.products.map((p: any) => {
    ...
    return {
      name: p.product_name_ar || p.product_name || "unknown product",
      code: p.code,
